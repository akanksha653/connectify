...
try {
  ...
  console.warn("⚠️ Firebase Admin init failed — rooms functionality will be limited.", err.message);
  // Do not throw; continue to allow 1-1 features to work
}
...
app.get("/rooms", async (req, res) => {
  if (!roomsCollection) return res.status(503).json({ error: "Rooms DB not available" });
  try {
...
// Hybrid: Firestore (persist) + Socket.io (realtime)
// -------------------------
const roomIO = io.of("/rooms");

roomIO.use((socket, next) => {
  // optional: allowlist check or auth here
...
roomIO.on("connection", async (socket) => {
  ...
  // helper to emit all rooms to all connected clients
  ...
  // send the current rooms snapshot immediately to the newly connected client
  if (roomsCollection) {
    try {
      const snapshot = await roomsCollection.get();
      const rooms = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
  ...
  socket.on("join-room", async ({ roomId, user }) => {
    ...
    try {
      ...
      // remove duplicate socket entry if exists
      ...
      // send updated users to everyone in room (including new user)
      socket.emit("room-users", users);
      roomIO.to(roomId).emit("room-users", users);

      // broadcast full rooms list update
      await emitRooms();

      // notify others in the room about the new user (useful for WebRTC)
  ...
  socket.on("leave-room", async ({ roomId }) => {
    ...
    try {
      ...
      // update room participants and broadcast rooms list
      roomIO.to(roomId).emit("room-users", users);
  ...
  socket.on("delete-room", async ({ roomId }) => {
    ...
    try {
      const roomRef = roomsCollection.doc(roomId);
      const doc = await roomRef.get();
      if (!doc.exists) return socket.emit("delete-room-error", { message: "Room not found" });
  ...
  // ---------- messaging (IMPROVED with subcollection) ----------
  ...
  socket.on("disconnect", async (reason) => {
    ...
    try {
      ...
      for (const docSnap of snapshot.docs) {
        ...
        const users = (docSnap.data()?.users || []).filter((u) => u.socketId !== socket.id);
        if (users.length !== (docSnap.data()?.users || []).length) {
          await roomsCollection.doc(rId).update({ users });
          roomIO.to(rId).emit("room-users", users);
          roomIO.to(rId).emit("user-left", { userId: socket.id });
          broadcastRooms = true;
        }
      }
