...
export default function AnonymousChatRoom() {
  ...
  // NEW: mic muted state (used by ControlBar)
  ...
  // -------------------------
  // Mute/unmute local audio track when `muted` changes
  // -------------------------
  useEffect(() => {
    ...
    try {
      ...
      console.warn("Failed to toggle local audio tracks:", err);
    }
  }, [localStream, muted]);
  ...
  const handleStop = () => {
    ...
    // stop local stream on stop to free camera/mic
    try {
  ...
  const handleSkip = () => {
    // Ensure we leave/cleanup aggressively
    if (roomId) socket?.emit("leave-room", roomId);
    ...
    setIsOfferer(null);
    setSessionStarted(false);
    setLoading(true); // start-looking will set loading on match
    setPartnerInfo(null);
    socket?.emit("skip");
    // then re-start search
    socket?.emit("start-looking", {
      name: userInfo?.name,
      age: userInfo?.age,
      gender: userInfo?.gender,
  ...
  }, [socket, playSound, genderFilter, countryFilter, userInfo]);
      ...
      <div className="flex flex-col md:flex-row w-full h-full overflow-hidden bg-neutral-100 dark:bg-neutral-900">
        ...
        <div className="w-full md:w-[420px] flex flex-col border-t md:border-l border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 h-[34vh] md:h-full">
          ...
          <ControlBar
            ...
            // NEW: pass muted state & setter to match ControlBar signature
            muted={muted}
            setMuted={setMuted}
