...
export function useRoomWebRTC(localStream: MediaStream | null) {
  ...
  // Ref to store the user-joined handler
  const userJoinedHandlerRef = useRef<((data: { socketId: string }) => void) | null>(null);
  ...
  const joinRoom = (roomId: string) => {
    ...
    if (userJoinedHandlerRef.current) socket.off("user-joined", userJoinedHandlerRef.current);
    userJoinedHandlerRef.current = handleUserJoined;
  ...
  const leaveRoom = () => {
    ...
    if (socket) {
      ...
      if (userJoinedHandlerRef.current) {
        socket.off("user-joined", userJoinedHandlerRef.current);
        userJoinedHandlerRef.current = null;
      }
    }
